...
export function buildInviteEmail(data: InviteTemplateData): EmailContent {
  const { confirmUrl } = data;
  const safeHref = escapeHtmlAttr(confirmUrl);
  const safeUrl = escapeHtml(confirmUrl);

  return {
    subject: "Votre accès Atelier Nox — Confirmer votre invitation",
    html: `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="utf-8" />
...
<body style="margin:0;padding:0;background:#f5f0eb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#f5f0eb;padding:40px 16px;">
        ...
        <table width="540" cellpadding="0" cellspacing="0" role="presentation" style="max-width:540px;width:100%;background:#ffffff;border:1px solid #dedad4;">
          ...
          <tr>
            <td style="padding:40px;background:#fffaf0;">
              ...
              <p style="margin:0 0 20px;color:#101820;font-size:15px;line-height:1.65;">
                Vous avez été invité à accéder à votre espace client Atelier Nox. Cliquez sur le bouton ci-dessous pour confirmer votre accès et définir votre mot de passe.
              </p>
              <p style="margin:0 0 32px;color:#6b6660;font-size:13px;line-height:1.6;">
                Ce lien est valide pendant <strong>24 heures</strong>. Si vous n'attendiez pas cette invitation, vous pouvez ignorer ce message.
              </p>
    ...
    text: `Vous avez été invité à accéder à votre espace client Atelier Nox.

Cliquez sur le lien ci-dessous pour confirmer votre accès et définir votre mot de passe :
